...
// Normal miner to miner transaction
class Transaction
{
public:
    string txnID;
...
public:
    ...
    unordered_map<string, double> blockArrivalTime; // arrival time of the block
...
// Attacker's specific information
class Attacker
{
    /* state is the lead in the blockchain
     * -1 -> no lead and competition (state 0')
     * 0 -> no lead (state 0), mine on the last honest block
     * n = +ve num -> lead of <num> (state <num>) */
public:
    int state, blocksGenerated; // # of blocks generated by adversary
    Block *lastBlock;           // the block attacker is minning on, it can be private or honest
    queue<Block *> privateChain;
    unordered_map<string, int> stateMap; // state after generating/receiving a block

    Attacker() {}
    Attacker(Block *lastBlock) : state(0), lastBlock(lastBlock), blocksGenerated(0){};
};